...
class TianxiaButtons(unittest.TestCase):
    ...
    def test_visibility_requires_initialisation_and_emperor(self):
        for button in (LEAVE, ADD, DISMANTLE):
            with self.subTest(button=button):
                ...
                emperor = self.w.targets.pop("EmperorOfChina")
                self.assertFalse(self.w.visible(button))
                self.w.targets["EmperorOfChina"] = emperor
    ...
    def test_leave_confirm_rechecks_war_membership_and_office(self):
        for change in (lambda w: w.root.enemies.add("KRC"),
                       ...
                       lambda w: setattr(w, "emperor", "YAN")):
    ...
    def test_characterize_leave_retains_special_vassal_relation(self):
        # Records an existing policy edge; does not assert that it is desirable.
        self.w.root.overlord, self.w.root.subject_type = "CZH", "gdd_tianxia_vassal"
        self.assertTrue(self.w.enabled(LEAVE))
        self.w.choose("gdd_tianxia_territory.3")
        self.assertNotIn("zhx_member", self.w.root.flags)
        self.assertEqual(self.w.root.overlord, "CZH")
        self.assertEqual(self.w.root.subject_type, "gdd_tianxia_vassal")
    ...
    def test_dismantle_cleans_political_state_and_only_special_subjects(self):
        ...
        self.assertIn("empire_of_china_dismantled", self.w.global_flags)
        ...
        self.assertEqual(self.w.native_teardowns, ["CZH"])
