...
def main() -> None:
    ...
    require("gdd_tianxia_defense_refusal_paid" not in monthly,
            "automatic non-defence penalty must remain disabled")
    require("gdd_shift_authority_toward_feudatories_5_effect = yes" in (MOD / "common/on_actions/zhx_diplomacy_on_actions.txt").read_text(),
    ...
    require("which = zhx_external_extinction_mandate_queued value = 10" in annex,
    ...
    for token in ("is_rival = event_target:zhx_tianzi",
    ...
    for token in ("set_global_flag = zhx_tianxia_dismantled",
                  ...
                  "zhx_cancel_member_pending_actions = yes"):
    ...
    for token in ("NOT = { zhx_is_tianxia_polity = yes }",
                  "NOT = { zhx_is_tianzi = yes }", "is_at_war = yes",
    ...
    for hook in ("on_diplomatic_annex", "on_integrate", "on_annexed"):
        ...
        require(hook_body.count("gdd_apply_external_tianxia_annexation_mandate_loss_effect") == (1 if hook == "on_annexed" else 0),
    ...
    # Political and native Mandate roles are explicitly distinguished in the revised text.
    ...
    print("  external wartime annexation: queued -10; no automatic defence penalty")
