#!/usr/bin/env python3
"""Build the editable Blender source and render the Luoyang palace preview."""
...
from luoyang_palace_geometry import MATERIALS, build_palace  # noqa: E402
...
def create_material(name: str) -> bpy.types.Material:
    spec = MATERIALS[name]
    material = bpy.data.materials.new(name=f"GDD_{name}")
...
def build_scene() -> None:
    ...
    palace = build_palace()
    collection = bpy.data.collections.new("GDD Luoyang Palace")
    ...
    for name, surface in palace.surfaces.items():
        ...
        mesh = bpy.data.meshes.new(f"gdd_luoyang_{name}_mesh")
        mesh.from_pydata(vertices, [], faces)
        mesh.materials.append(materials[name])
        mesh.update()
        obj = bpy.data.objects.new(f"gdd_luoyang_{name}", mesh)
        collection.objects.link(obj)

    ground_material = bpy.data.materials.new("GDD_preview_ground")
    ground_material.diffuse_color = (0.24, 0.20, 0.13, 1.0)
    ground_material.use_nodes = True
    ground_shader = ground_material.node_tree.nodes.get("Principled BSDF")
    ground_shader.inputs["Base Color"].default_value = (0.24, 0.20, 0.13, 1.0)
    ground_shader.inputs["Roughness"].default_value = 0.96
    ...
    sun.rotation_euler = (math.radians(28.0), math.radians(-18.0), math.radians(-34.0))

    fill_data = bpy.data.lights.new("Fill", type="AREA")
    fill_data.energy = 820.0
    fill_data.shape = "DISK"
    fill_data.size = 8.0
    fill = bpy.data.objects.new("Fill", fill_data)
    bpy.context.scene.collection.objects.link(fill)
    fill.location = (-8.0, -5.0, 10.0)
    look_at(fill, (0.0, 0.0, 1.0))

    camera_data = bpy.data.cameras.new("Camera")
    camera = bpy.data.objects.new("Camera", camera_data)
    bpy.context.scene.collection.objects.link(camera)
    camera.location = (11.6, -15.8, 12.4)
